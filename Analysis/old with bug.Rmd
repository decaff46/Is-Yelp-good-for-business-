...
'Goan Catholic', 'Greek', 'Gujarati', 'Hyderabad', 'Indian', 'Indian Chinese', 'Singaporean', 'Indonesian', 'Inuit', 'Italian American', 'Italian', 'Japanese', 'Jewish', 'Karnataka', 'Kazakh', 'Keralite', 'Korean', 'Kurdish', 'Laotian','Latvian', 'Lithuanian', 'Louisiana Creole', 'Maharashtrian', 'Mangalorean', 'Malay', 'Chinese', 'Malaysian', 'Indian', 'Mediterranean', 'Mexican', 'Mordovian', 'Mughal','Native American', 'Nepalese', 'New Mexican', 'Odia', 'Parsi', 'Pashtun', 'Polish', 'Pennsylvania Dutch', 'Pakistani', 'Peranakan', 'Persian', 'Peruvian', 'Portuguese', 'Punjabi', 'Rajasthani', 'Romanian', 'Russian', 'Sami', 'Serbian', 'Sindhi', 'Slovak', 'Slovenian', 'Somali', 'South Indian', 'Sri Lankan', 'Taiwanese', 'Tatar', 'Thai', 'Turkish', 'Tamil', 'Udupi', 'Ukrainian', 'Yamal', 'Zanzibari')
# reference: https://en.wikipedia.org/wiki/List_of_cuisines

this.cate = strsplit(restaurant$categories, ",")
restaurant.category = data.table(Name = unlist(this.cate))
new.restaurant.category = restaurant.category[,.(Count = .N),by = Name]
setorderv(x = new.restaurant.category, cols = 'Count', order = -1)  
datatable(new.restaurant.category, rownames = F)

cate.name <- new.restaurant.category[,unique(Name)]
pc.list <- c()
for (i in cuisine.list){
  cuisine <- cate.name[grep(pattern = i, x = cate.name)]
  pc.list <- append(pc.list, cuisine)
}
cuisine.tab <- new.restaurant.category[Name %in% pc.list,]
datatable(cuisine.tab, rownames = F)


# Visualize the Top 10 cuisine
top10_cuisine_plot = ggplot(data = cuisine.tab[1:10], aes(x = reorder(Name, Count), y = Count, fill = Name)) +
  geom_bar(stat = "identity", color = "black") +
...
Italian, 4K, and Chinese, 5K, are the most famous cuisine in the restaurants. Let's now find more information about locations : state and city. 

```{r Exploring City and State }
# business by state
state.tab = restaurant[, .(Count = .N), by = state]
...
# business by city
city.tab = restaurant[, .(Count = .N), by = city]
...
```{r Cities in Arizona}
AZ_cities_numb = restaurant[get(state.name) == "AZ", length(unique(get(city.name)))]
print(AZ_cities_numb) # 57

NV_cities_numb = restaurant[get(state.name) == "NV", length(unique(get(city.name)))]
print(NV_cities_numb) # 23

AZ_NV_restaurant_numb = restaurant[get(state.name) == "AZ" | get(state.name) == "NV", .N]
...
overall.surviving.rate = restaurant[, .N, by = is.open.name]
...
vegas.open.tab = restaurant[get(city.name) == "Las Vegas", .N , by = is.open.name]
phoenix.open.tab = restaurant[get(city.name) == "Phoenix", .N , by = is.open.name]
